...
## The DialogueComponent for the current speaker, if any, for per-letter talk sounds
var speaker: DialogueComponent
...
func start(
	...
	speaker = extra_game_states.filter(
		func(s):
			return s is DialogueComponent,
	).front() if extra_game_states.any(
		func(s):
			return s is DialogueComponent,
	) else null
...
func _on_dialogue_label_spoke(letter: String, letter_index: int, _speed: float) -> void:
	if not speaker:
		return
	speaker.play_talk_letter(letter, letter_index)
#endregion
