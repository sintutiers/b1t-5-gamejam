...
static func get_copy_path() -> String:
	var copy_path = ProjectSettings.get_setting(
		PROJECT_SETTINGS_PATH + "copy_path",
		get_plugin_examples_path(),
	)
...
func open_theme_selection_dialog(target_path: String) -> void:
	selected_theme = ""
	var theme_selection_scene: PackedScene = load(
		get_plugin_path() + "installer/theme_selection_dialog.tscn"
	)
	...
	theme_selection_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(theme_selection_instance)
	)
...
func open_setup_complete_dialog(_target_path: String) -> void:
	var setup_complete_scene: PackedScene = load(
		get_plugin_path() + "installer/setup_complete_dialog.tscn"
	)
	var setup_complete_instance: AcceptDialog = setup_complete_scene.instantiate()
	setup_complete_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(setup_complete_instance)
	)
...
func open_main_scene_confirmation_dialog(target_path: String) -> void:
	var main_confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/main_scene_confirmation_dialog.tscn"
	)
	...
	main_confirmation_instance.confirmed.connect(
		_update_main_scene.bind(target_path, new_main_scene_path)
	)
	main_confirmation_instance.canceled.connect(
		_delayed_open_setup_complete_dialog.bind(target_path)
	)
	main_confirmation_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(main_confirmation_instance)
	)
	add_child(main_confirmation_instance)


func open_delete_examples_short_confirmation_dialog() -> void:
	var delete_confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/delete_examples_short_confirmation_dialog.tscn"
	)
	...
	delete_confirmation_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(delete_confirmation_instance)
	)
...
func open_input_icons_dialog() -> void:
	var input_icons_scene: PackedScene = load(
		get_plugin_path() + "installer/kenney_input_prompts_installer.tscn"
	)
...
func open_copy_and_edit_dialog() -> void:
	var copy_and_edit_scene: PackedScene = load(
		get_plugin_path() + "installer/copy_and_edit_files.tscn"
	)
...
func _open_play_opening_confirmation_dialog(target_path: String) -> void:
	var play_confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/play_opening_confirmation_dialog.tscn"
	)
	...
	play_confirmation_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(play_confirmation_instance)
	)
...
func _open_delete_examples_confirmation_dialog(target_path: String) -> void:
	var delete_confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/delete_examples_confirmation_dialog.tscn"
	)
	var delete_confirmation_instance: ConfirmationDialog = delete_confirmation_scene.instantiate()
	delete_confirmation_instance.confirmed.connect(
		_delete_source_examples_directory.bind(target_path)
	)
	delete_confirmation_instance.canceled.connect(
		_check_main_scene_needs_updating.bind(target_path)
	)
	delete_confirmation_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(delete_confirmation_instance)
	)
...
func _update_app_config_paths(target_path: String) -> void:
	...
	var prefixes: Array[String] = ["main_menu_scene_path", "game_scene_path", "ending_scene_path"]
...
func _update_scene_loader_path(target_path: String) -> void:
	...
	var regex := RegEx.create_from_string(
		"loading_screen_path = \"(\\S*)%s.tscn\"" % path_for_regex
	)
	var replacement: String = "loading_screen_path = \"%s%s\"" % [
		target_path,
		LOADING_SCREEN_SCENE_RELATIVE_PATH,
	]
...
func _add_translations() -> void:
	var dir := DirAccess.open("res://")
	var translations: PackedStringArray = ProjectSettings.get_setting(
		"internationalization/locale/translations",
		[],
	)
	for available_translation in AVAILABLE_TRANSLATIONS:
		var translation_path = get_plugin_path() + ("base/translations/menus_translations.%s.translation"
...
func _open_confirmation_dialog() -> void:
	var confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/copy_confirmation_dialog.tscn"
	)
	...
	confirmation_instance.visibility_changed.connect(
		_on_visibility_changed_to_hidden.bind(confirmation_instance)
	)
...
func _open_continue_setup_dialog() -> void:
	var confirmation_scene: PackedScene = load(
		get_plugin_path() + "installer/continue_setup_confirmation_dialog.tscn"
	)
	var confirmation_instance: ConfirmationDialog = confirmation_scene.instantiate()
...
func _open_check_plugin_version() -> void:
	if ProjectSettings.has_setting(PROJECT_SETTINGS_PATH + "disable_update_check"):
		...
		ProjectSettings.save()
	var check_version_scene: PackedScene = load(
		get_plugin_path() + "installer/check_plugin_version.tscn"
	)
...
func _remove_update_plugin_tool_option() -> void:
	if update_plugin_tool_string.is_empty():
		return
...
func _install_audio_busses() -> void:
	if not ProjectSettings.get_setting(
		PROJECT_SETTINGS_PATH + "disable_install_audio_busses",
		false,
	):
...
func _enable_plugin():
	...
	add_autoload_singleton(
		...
		get_plugin_path() + "base/nodes/autoloads/music_controller/project_music_controller.tscn",
	)
	add_autoload_singleton(
		"ProjectUISoundController",
		get_plugin_path()
		+ "base/nodes/autoloads/ui_sound_controller/project_ui_sound_controller.tscn",
	)
