...
func _setup_processing(processing_enabled: bool) -> void:
	process_mode = Node.PROCESS_MODE_ALWAYS if processing_enabled else Node.PROCESS_MODE_DISABLED
	visible = processing_enabled
