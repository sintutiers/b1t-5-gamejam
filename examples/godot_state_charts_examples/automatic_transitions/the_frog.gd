...
func _on_can_walk_state_physics_processing(delta: float) -> void:
	# Get the direction
...
func _on_needs_rest_state_physics_processing(delta: float) -> void:
	stamina = min(100, stamina + RECOVER_RATE * delta)


### ANIMATION CONTROL STATES
func _on_pulsating_red_state_entered() -> void:
